...
resource "aws_iam_role_policy_attachment" "CodePipelinePolicyAttachmet" {
...
resource "aws_codepipeline" "myCodePipeline" {
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        S3ObjectKey = aws_s3_object.file_upload.key
      }
    }
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ApplicationName     = aws_codedeploy_app.myWebApp.name
        DeploymentGroupName = aws_codedeploy_deployment_group.myDeploymentGroup.deployment_group_name
      }
    }
  }
}
